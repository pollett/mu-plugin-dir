 ...
 * Description: Loads plugins from mu-plugins/pluginname/, won't trigger activation hooks
 * Version: 0.2
...
class MUPluginLoader
{
 ...
 public static function init() {
  ...
  add_action( 'admin_init', function() {
   add_action( 'after_plugin_row_mu-plugins.php', array( __CLASS__, 'show_loaded_plugins' ) );
...
//  return substr( $fulldir, strpos( $fulldir, '/themes' ) );
  return '/mu-plugins/' . $name;
 }
 
 public static function get_mu_plugins(){
  $plugins = get_site_transient( 'mu-plugins' );
  ...
  set_site_transient( 'mu-plugins', $plugins );
  return $plugins;
 }
 
 public static function clear_cache(){
  delete_site_transient( 'mu-plugins' );
 }

 public static function load_plugins(){
  foreach ( self::get_mu_plugins() as $plugin_file ) {
 ...
 public static function plugins_url( $url = '', $path = '', $plugin = '' ) {
  static $content_dir, $mu_dir, $mu_url;
  ...
  if ( ! $plugin || 0 !== strpos( $plugin, $mu_dir ) ) {
   return $url;
  }

  if ( 0 === strpos( $plugin, $mu_dir ) )
   $url_override = str_replace( $mu_dir, $mu_url, dirname( $plugin ) );
